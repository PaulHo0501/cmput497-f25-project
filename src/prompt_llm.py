...
PER_USER_PROMPT_TEMPLATE = '''
You are tasked with evaluating responses with size {text_list_size} based on two scoring rubrics that serve as the evaluation standards. Provide a comprehensive feedback strictly adhering to the scoring rubrics, without any general evaluation. Follow this with two scores, each between -2 and 2, referring to the scoring rubric. Avoid generating any additional opening, closing, or explanations.
  ...
  **Result:** <pairs of scores, each between -2 and 2, separated by a comma per scores, there are {text_list_size} pairs >
...
def evaluate_per_user(args):
    ...
    with open(LOGS_PATH/"per_user.txt", 'w', encoding='utf-8') as log_file:
        for user_id, text_list in tqdm(grouped.items()):
            len_text = len(text_list)
            # Format all texts line by line
            response_text = "\n".join(text_list)
